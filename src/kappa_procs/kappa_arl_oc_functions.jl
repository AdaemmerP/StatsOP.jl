...
function arl_kappa_oc(
  qual_dgp, qual_null_dist, lam, cl, reps; chart_choice, d=1
)
  ...
  elseif nprocs() == 1 && reps > Threads.nthreads()
    ...
    par_results = map(chunks) do i
      ...
      Threads.@spawn rl_gop_oc(lam, cl, i, qual_dgp, qual_dgp.dist, qual_null_dist, chart_choice, d)
  ...
  elseif nprocs() > 1 && reps >= nworkers()
    ...
    par_results = pmap(chunks) do i
      rl_gop_oc(lam, cl, i, qual_dgp, qual_dgp.dist, qual_null_dist, chart_choice, d)
    end
...
function rl_kappa_oc(
  ...
  # Initialize at t = 0
  qₜ = cdf(qual_null_dist, support(qual_null_dist))
  Qₜ = sum(qₜ .^ 2)
...
function rl_kappa_oc(
  ...
  # Initialize at t = 0
  f₀ = cdf(qual_null_dist, support(qual_null_dist))
  Qₜ = sum(f₀ .^ 2)
