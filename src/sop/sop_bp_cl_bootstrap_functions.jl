...
# Example
```julia
cl = cl_sop_bp_bootstrap(data, 0.1, 370.0, 2.5, 3; reps_final=50_000, seed=42)
```
"""
function cl_sop_bp_bootstrap(
    data::Array{T,3}, lam, L0, cl_init, w;
    reps_final=10_000,
    reps_bracket=1_000,
    bracket_step=0.001,
    ...
    function get_arl(cl, current_reps, current_truncate)
        Random.seed!(seed)
        res = arl_sop_bp_bootstrap(
            p_array, lam, cl, w, current_reps;
