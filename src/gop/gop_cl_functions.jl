...
Compute the critical limit `cl` for an EWMA control chart based on generalized ordinal
patterns (GOP) such that the in-control ARL equals `L0`.
...
function cl_gop(
    ...
    function get_arl(cl, current_reps, current_truncate)
        Random.seed!(seed)
        res = arl_gop_ic(
            gop_dgp, lam, cl, current_reps;
            chart_choice=chart_choice, d=d, rl_max=current_truncate
        )
        if verbose
