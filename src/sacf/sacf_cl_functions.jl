...
Compute the critical limit `cl` for an EWMA control chart based on the spatial
...
"""
function cl_sacf(
    sp_dgp::ICSTS, lam, L0, cl_init, d1::Int, d2::Int;
    reps_final=10_000,
    reps_bracket=1_000,
    bracket_step=0.01,
    ...
    function get_arl(cl, current_reps, current_truncate)
        Random.seed!(seed)
        res = arl_sacf_ic(sp_dgp, lam, cl, d1, d2, current_reps; rl_max=current_truncate)
        if verbose
