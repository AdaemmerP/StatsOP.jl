...
Compute the critical limit `cl` for an EWMA control chart based on the
autocorrelation function (ACF) such that the in-control ARL equals `L0`.
...
# Arguments
- `acf_dgp::Union{IC, AR1, TEAR1}`: Data-generating process under the in-control
  distribution.
...
function cl_acf(
  ...
  function get_arl(cl, current_reps, current_truncate)
    Random.seed!(seed)
    res = arl_acf_ic(lam, cl, acf_dgp, current_reps, acf_version; rl_max=current_truncate)
    if verbose
