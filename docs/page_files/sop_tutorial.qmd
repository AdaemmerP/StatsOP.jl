...
- **`stat_sop(data, d1, d2; chart_choice, refinement=OrdinaryType())`**
    ...
    - `refinement`: `OrdinaryType()` for classical types, or `RotationType()`, `DirectionType()`, `DiagonalType()`.

- **`test_sop(data, d1, d2; chart_choice, refinement=OrdinaryType(), alpha=0.05)`**
...
- **`bootstrap_sop(data, n_boot, d1, d2; chart_choice, refinement=OrdinaryType(), block_size=1)`**
...
- **`test_sop_bootstrap(data, n_boot, d1, d2; chart_choice, refinement=OrdinaryType(), alpha=0.05, block_size=1)`**
...
```{julia}
test_sop(data_iid, d1, d2; chart_choice=Shannon())
```

```{julia}
test_sop(data_sar, d1, d2; chart_choice=Shannon())
```
