...
arlse_sacf_sar22_mat = similar(arl_sacf_sar22_mat)
arl_sop_sar22_mat = similar(arl_sacf_sar22_mat)
arlse_sop_sar22_mat = similar(arl_sacf_sar22_mat)
...
for (i, MN) in enumerate(MN_vec)
    ...
    for (j, d1d2) in enumerate(d1d2_vec)
        ...
        arlse_sacf_sar22_mat[i, j] = sacf_results[2]
        ...
        arlse_sop_sar22_mat[i, j] = sop_results[2]
...
arlse_sacf_sar22_outl_mat = similar(arl_sacf_sar22_outl_mat)
arl_sop_sar22_outl_mat = similar(arl_sacf_sar22_outl_mat)
arlse_sop_sar22_outl_mat = similar(arl_sacf_sar22_outl_mat)
...
for (i, MN) in enumerate(MN_vec)
    ...
    for (j, d1d2) in enumerate(d1d2_vec)  
        ...
        arlse_sacf_sar22_outl_mat[i, j] = sacf_results[2]
        ...
        arlse_sop_sar22_outl_mat[i, j] = sop_results[2]
...
arlse_sacf_sar22_bp_mat = similar(arl_sacf_sar22_bp_mat)
arl_sop_sar22_bp_mat = similar(arl_sacf_sar22_bp_mat)
arlse_sop_sar22_bp_mat = similar(arl_sacf_sar22_bp_mat)
...
arlse_sacf_sar22_outl_bp_mat = similar(arl_sacf_sar22_outl_bp_mat)
arl_sop_sar22_outl_bp_mat = similar(arl_sacf_sar22_outl_bp_mat)
arlse_sop_sar22_outl_bp_mat = similar(arl_sacf_sar22_outl_bp_mat)
...
for (i, MN) in enumerate(MN_vec)
    ...
    for w in 1:w_max
        ...
        arlse_sacf_sar22_outl_bp_mat[i, w] = sacf_results[2]
        ...
        arlse_sop_sar22_outl_bp_mat[i, w] = sop_results[2]
        println("Progress -> SAR(2, 2) with outliers: i: $i, w: $w")
    end
end


# compare results with Table A.14
hcat(arl_sop_sar22_mat, arl_sacf_sar22_mat, 
     arl_sop_sar22_outl_mat, arl_sacf_sar22_outl_mat)

# Maximum ARL standard error
findmax([
    round.(arlse_sop_sar22_mat, digits=2);
    round.(arlse_sacf_sar22_mat, digits=2);
    round.(arlse_sop_sar22_outl_mat, digits=2);
    round.(arlse_sacf_sar22_outl_mat, digits=2)
    ])

# compare results with Table A.15

# ARL results
hcat(arl_sop_sar22_bp_mat, arl_sacf_sar22_bp_mat, 
     arl_sop_sar22_outl_bp_mat, arl_sacf_sar22_outl_bp_mat)

# Maximum ARL standard error
findmax([    
        round.(arlse_sop_sar22_bp_mat, digits=2);
        round.(arlse_sacf_sar22_bp_mat, digits=2);
        round.(arlse_sop_sar22_outl_bp_mat, digits=2);
        round.(arlse_sacf_sar22_outl_bp_mat, digits=2)
])
