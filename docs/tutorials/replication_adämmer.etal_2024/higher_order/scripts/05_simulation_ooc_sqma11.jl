...
arlse_sacf_sqma11_mat = similar(arl_sacf_sqma11_mat)
arl_sop_sqma11_mat = similar(arl_sacf_sqma11_mat)
arlse_sop_sqma11_mat = similar(arl_sacf_sqma11_mat)
...
for (i, MN) in enumerate(MN_vec)
    ...
    for (j, d1d2) in enumerate(d1d2_vec)
        ...
        arlse_sacf_sqma11_mat[i, j] = sacf_results[2]
        ...
        arlse_sop_sqma11_mat[i, j] = sop_results[2]
...
arlse_sacf_bp_sqma11_mat = similar(arl_sacf_bp_sqma11_mat)
arl_sop_bp_sqma11_mat = similar(arl_sacf_bp_sqma11_mat)
arlse_sop_bp_sqma11_mat = similar(arl_sacf_bp_sqma11_mat)
...
for (i, MN) in enumerate(MN_vec)
    ...
    for w in 1:w_max
        ...
        arlse_sacf_bp_sqma11_mat[i, w] = sacf_results[2]

        # Compute ARL for SOP for SQMA(1, 1)
        sop_results = arl_sop_bp_oc(sqma11, lam, cl_sop_bp_mat[i, w], w, reps; chart_choice=3)
        arl_sop_bp_sqma11_mat[i, w] = sop_results[1]
        arlse_sop_bp_sqma11_mat[i, w] = sop_results[2]
        println("Progress -> BP-SQMA(1, 1): i: $i, w: $w")
    end
end

# compare results with Table A.16

# ARL results
hcat(arl_sop_sqma11_mat, arl_sacf_sqma11_mat, arl_sop_bp_sqma11_mat, arl_sacf_bp_sqma11_mat)

# Maximum ARL standard error
findmax([    
        round.(arlse_sop_sqma11_mat, digits=2);
        round.(arlse_sacf_sqma11_mat, digits=2);
        round.(arlse_sop_bp_sqma11_mat, digits=2);
        round.(arlse_sacf_bp_sqma11_mat, digits=2);
])
