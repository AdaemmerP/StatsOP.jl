...
prerun = 100
μ = 5   # for Poisson distribution
p = 0.1
γ = 5*μ
dist = Poisson(μ)
dist_ao = PoiBin(p, γ) # additive outlier distribution
...
for alphas in dgp_vals
    for (j, mn_tup) in enumerate(MN_vec)
        ...
        # Set up DGP SINAR(1, 1) with additive outliers
        ...
        println("Progress -> SINAR(1, 1) contaminated: grid: $j, alphas: $alphas")
        global i = i + 1
    end
end

# compare results with Table A.4
