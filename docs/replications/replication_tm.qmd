...
```{julia}
data = randn(43, 28, 370);
lam = 0.1
L0 = 370
d1_vec = [1, 2]
d2_vec = [1, 2]
spatial_dgp = ICSP(28, 28, Normal(0, 1))

cl_init = stat_sop(lam, data, d1_vec, d2_vec; chart_choice=3, add_noise=false) |> x -> last(x)
cl_sop(lam, L0, spatial_dgp, cl_init, d1_vec, d2_vec, 100; chart_choice=3, jmin=4, jmax=6, verbose=true)

arl_sop(lam, 0.1020, spatial_dgp, d1_vec, d2_vec, 100; chart_choice=3)

spatial_dgp = SAR11((0.1, 0.1, 0.1), 28, 28, Normal(0, 1), nothing, 100)

arl_sop(lam, 0.1020, spatial_dgp, d1_vec, d2_vec, 1_000; chart_choice=3)
```