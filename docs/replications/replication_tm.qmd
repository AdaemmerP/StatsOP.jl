...
L0 = 370
cl_init = 0.01
cl_sop(0.1, L0, sop_dgp, cl_init, 1000; chart_choice=1, jmin=4, jmax=6, verbose=true, d1=1, d2=1)


# Verify ARL
lam = 0.1
cl = 0.03049
arl_sop(lam, cl, sop_dgp, reps; chart_choice=1, d1=1, d2=1)

# Limits SACFs
cl_sacf(lam, L0, sop_dgp, cl_init, reps; jmin=4, jmax=6, verbose=true, d1=2, d2=2)
```
...
for alphas in dgp_vals
    for (j, mn_tup) in enumerate(MN_vec)
        ...
        for chart in 1:4
            ...
            res_sacf = arl_sacf(lam, cl_sacf, sop_dgp, reps; d1=1, d2=1)
...
stat_sop_bp(0.1, data; chart_choice=3, add_noise=false, d1_vec=[1], d2_vec=[1])

stat_sop(0.1, data; chart_choice=3, add_noise=false, d1=1, d2=1)

stat_sop(0.1, data, [1, 1], [1, 1]; chart_choice=3, add_noise=false)

stat_sop(data, d1_vec::Vector{Int}, d2_vec::Vector{Int}; chart_choice)
 
stat_sop(0.1, data, 1, 1; chart_choice=3, add_noise=false) |> x->quantile(x, 0.99)

sp_dgp = ICSP(25, 25, Normal(0, 1))

cl_sop(0.1, 370, sp_dgp, 0.08, 100, [1, 1], [1, 1]; chart_choice=3, jmin=4, jmax=6, verbose=false)
```